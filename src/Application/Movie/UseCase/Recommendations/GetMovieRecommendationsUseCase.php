...
namespace App\Application\Movie\UseCase\Recommendations;

use App\Application\Movie\DTO\GetMovieRecommendationsResponse;
...
/**
 * Endpoint dedicado para recomendações de filme — separado de
 * GetMovieDetailsUseCase. Chama diretamente
 * TmdbApiService::getMovieRecommendations(), sem buscar detalhes
 * completos do filme antes.
 */
final readonly class GetMovieRecommendationsUseCase {
    /**
     ...
     * @param int $movieId
     *
     * @return \App\Application\Movie\DTO\GetMovieRecommendationsResponse
     * @throws \App\Domain\Exception\ExternalServiceException
     */
    public function execute(int $movieId): GetMovieRecommendationsResponse {
        try {
            $result = $this->tmdbApiService->getMovieRecommendations($movieId);

            // NOTA: /movie/{id}/recommendations é resposta paginada
            // (page/results/total_pages/total_results), sem "id" no
            // topo — diferente de credits/images/videos. Validação
            // segue o mesmo padrão de SearchMoviesUseCase::execute().
            if (!isset($result['results']) || !is_array($result['results'])) {
                $this->logger->error(
                    'Invalid TMDB API response structure for movie recommendations',
                    [
                        'movie_id' => $movieId,
            ...
            return GetMovieRecommendationsResponse::success(
            ...
            $this->logger->error(
                'Client error fetching movie recommendations',
                ['movie_id' => $movieId, 'error' => $e->getMessage()],
            );
            ...
            $this->logger->error(
                'Server error fetching movie recommendations',
                ['movie_id' => $movieId, 'error' => $e->getMessage()],
            );
            ...
            $this->logger->error(
                'Unexpected error fetching movie recommendations',
                [
                    'movie_id' => $movieId,
                    'error' => $e->getMessage(),
